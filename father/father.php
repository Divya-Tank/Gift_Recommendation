...
<!-- birthdaysection  -->
        ...
        <?php  
          $conn=mysqli_connect("localhost",'root','','father') or die("connection faild!");
...
<!-- anniversary...section here start       -->
        ...
        <div class="gallery-section">
            <div class="gallery-wrap">
                ...
                <div class="gallery-images">
                    <?php  
          $conn=mysqli_connect("localhost",'root','','father') or die("connection faild!");
         $res=mysqli_query($conn,"SELECT * FROM `anniversary` ORDER BY `anniversary`.`like` DESC") or die("Query error!");
if(isset($_POST['submit_like']))
         {
             $id=$_POST['id'];
             $sq="SELECT `like` FROM `anniversary` WHERE  id={$id}";
             $res=mysqli_query($conn,$sq) or die('querry error!!');
             $pre_like=mysqli_fetch_assoc($res);
             $pre_like=$pre_like['like']+1;
             $sql="UPDATE `anniversary` SET `like`='{$pre_like}' WHERE id=$id";
           if(mysqli_query($conn,$sql) or die("Query erroro"))
           {
               header("Location: ./father.php");
           }  
...
<!-- congratulationsection -->
        ...
        <?php  
                  $conn=mysqli_connect("localhost",'root','','father') or die("connection faild!");
                  $res=mysqli_query($conn,"SELECT * FROM `congratulation` ORDER BY `congratulation`.`like` DESC") or die("Query error!");
                  if(isset($_POST['submit_like']))
                  {
                      $id=$_POST['id'];
                      $sq="SELECT `like` FROM `congratulation` WHERE  id={$id}";                    
                      $res=mysqli_query($conn,$sq) or die('querry error!!');
                      $pre_like=mysqli_fetch_assoc($res);
                      $pre_like=$pre_like['like']+1;            
                    $sql="UPDATE `congratulation` SET `like`='{$pre_like}' WHERE id=$id";
                    if(mysqli_query($conn,$sql) or die("Query erroro"))
                    {
                        header("Location: ./father.php");
                    }  
